...
%% 保存数据
 NBS=4;%台站数量
 shape='方阵';%布站形状
 boundary=['B:',num2str(BMIN),'_',num2str(BMAX),',','L:',num2str(LMIN),'_',num2str(LMAX)];%定位仿真范围；
 %save(['result\mat\布站_4站_武汉中心1000公里方阵_,定位范围',boundary,',noise_',num2str(NOISE),'us,ntime_',num2str(ntime),'.mat']);
 save('result\mat\布站_5站武汉中心1000公里方阵,范围_武汉_边长_4000_11000km_精度_1度_noise_200us_ntime_1000.mat');
 %% 均方根误差分布图
 if(0)
 figure
[X,Y]=meshgrid(L,B);
